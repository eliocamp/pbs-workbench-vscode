...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("workbench.start", async () => {
    ...
    vscode.commands.registerCommand("workbench.connectNewWindow", async () => {
      ...
      if (info) await connectRemoteSSH(info.hostname, info.jobdir, true);
    }),

    vscode.commands.registerCommand("workbench.connectCurrentWindow", async () => {
      const info = manager.cachedInfo;
      if (info) await connectRemoteSSH(info.hostname, info.jobdir, false);
    }),
  );
...
async function connectRemoteSSH(node: string, jobdir: string, newWindow: boolean): Promise<void> {
